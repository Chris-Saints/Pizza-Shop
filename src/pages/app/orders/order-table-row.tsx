...
import type { GetOrdersResponses } from "@/api/get-orders";
import { approveOrder } from "@/api/approve-order";
import { deliverOrder } from "@/api/deviver-order";
...
export function OrderTableRow({ order }: OrderTableRowProps) {
    ...
    function uptadeStatusOrderChache(orderId: string, status: OrderStatus) {
        const orderListCache = queryClient.getQueriesData<GetOrdersResponses>({
            queryKey: ['orders'],
        })
        ...
        orderListCache.forEach(([cacheKey, cacheData]) => {
            ...
            queryClient.setQueryData<GetOrdersResponses>(cacheKey, {
                ...cacheData,
    ...
    const {mutateAsync: cancelOrderFn, isPending: isCancelingOrder} = useMutation({
        ...
        async onSuccess(_, {orderId}) {
            uptadeStatusOrderChache(orderId, 'canceled')
        }
    })
    ...
    const {mutateAsync: approveOrderFn, isPending: isApprovingOrder} = useMutation({
        ...
        async onSuccess(_, {orderId}) {
            uptadeStatusOrderChache(orderId, 'processing')
        }
    })
    ...
    const {mutateAsync: dispatchOrderFn, isPending: isDispatchingOrder} = useMutation({
        ...
        async onSuccess(_, {orderId}) {
            uptadeStatusOrderChache(orderId, 'delivering')
        }
    })
    ...
    const {mutateAsync: deliverOrderFn, isPending: isDeliveringOrder} = useMutation({
        ...
        async onSuccess(_, {orderId}) {
            uptadeStatusOrderChache(orderId, 'delivered')
        }
    })
